<x-app-layout>
    <x-slot name="header">
        <h1 class="font-['Manrope'] text-[19px] font-extrabold m-0">Record Stock In (Delivery)</h1>
        <div class="text-[12.5px] text-[#5B6472] mt-[2px]">Log supplier shipments to safely increment inventory stock</div>
    </x-slot>

    <form method="POST" action="{{ route('stock-ins.store') }}" class="bg-white border border-[#E5E9EF] rounded-[16px] p-6 max-w-4xl">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4 mb-5">
            ...
            <div>
                <label for="quantity_received" class="block text-[11.5px] font-semibold text-[#5B6472] mb-1.5">Filled Tanks Received <span class="text-[#B5504B]">*</span></label>
                <input type="number" name="quantity_received" id="quantity_received" value="{{ old('quantity_received') }}" min="0" required class="w-full px-3 py-2.5 border border-[#E5E9EF] rounded-lg text-[14px] focus:ring-[#0B3B70] focus:border-[#0B3B70]" placeholder="0">
            ...
            <div>
                ...
                <input type="number" name="empty_returned_qty" id="empty_returned_qty" value="{{ old('empty_returned_qty', 0) }}" min="0" required class="w-full px-3 py-2.5 border border-[#E5E9EF] rounded-lg text-[14px] focus:ring-[#0B3B70] focus:border-[#0B3B70]" placeholder="0">
